...
import {
    addToOrders, capitalizeFirstLetter,
    createPaymentCheckout,
    findIfEmailExist,
    findIfEmailExistAndIsVerified, getProductRange, unCapitalizeFirstLetter, updateUserCart,
    validateUser
} from "../Helpers/helpers";
import Review from "../Models/Review";
import { closeAccount, forgotPassword, sendExperience, verifyEmail } from "../Helpers/verifyUser";
...
import Notification, { INotication } from "../Models/Notication";
...
router.post('/user/reset', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const otp = await jwt.sign({ _id: user._id.toString() }, process.env.JWT_SECRET, {
            expiresIn: '1h'
        });
...
router.post('/user/order', auth, async (req: Request, res: Response) => {
    ...
    try {
        ...
        product.save();
        const order = new Order({ ...req.body, sellerId: product!.owner, userId: req.user._id });
        await order.save()
    ...
    try {
        ...
        if (cart) {
            ...
            if (productIndex > -1) {
                ...
                if (product.quantity + parseInt(quantity) > productQuantity) return res.status(400).send('Product quantity exceeded')
            ...
            cart.products.push({ productId, quantity, name: title, price, photo, variants })
            cart.bill = cart.products.reduce((acc, cur) => acc + cur.quantity * cur.price, 0);
            await cart.save()
            return res.status(200).send(cart)
        }
...
router.post('/user/refund', auth, async (req: Request, res: Response) => {
    ...
    try {
        const isVerified: IOrder | null = await Order.findOne({ userId: req.user._id, productId: _id });
...
router.get('/user/order', auth, async (req: Request, res: Response) => {
    ...
    try {
        const order: IOrder[] = await Order.find({ userId: req.user._id, status }).populate({
            path: 'productId',
            model: Product,
            populate: {
                path: 'ratingId',
...
router.get('/user/order/shipped', auth, async (req: Request, res: Response) => {
    try {
        const order: IOrder[] = await Order.find({ userId: req.user._id }).or([{ status: 'shipped' }, { status: 'delivered' }]).populate({
            path: 'productId',
            model: Product,
            populate: {
                path: 'ratingId',
...
router.get('/user/order/cancelled', auth, async (req: Request, res: Response) => {
    try {
        const order: IOrder[] = await Order.find({ userId: req.user._id }).or([{ status: 'cancelled' }, { status: 'processed' }]).populate({
            path: 'productId',
            model: Product
        });
        res.status(200).send(order);
...
router.post('/user/rate', auth, async (req: Request, res: Response) => {
    ...
    try {
        const rating: IRating | null = await Rating.findOne({ productId: new ObjectId(data.productId) });
        if (rating) {
            const existingUser = rating.ratings.find(data => data.userId.toHexString() === req.user._id.toHexString());
            if (existingUser) {
                return res.status(401).send({ status: 'User cant rate the same item more than once' })
            }
            rating.ratings = rating.ratings.concat({ rating: data.rating, name: req.user.email, userId: req.user._id, comment: req.body.comment });
            const newRating: number = rating.ratings.reduce((acc, obj) => acc + obj.rating, 0);
...
router.patch('/user/notification/read', auth, async (req: Request, res: Response) => {
    try {
        const notifications: INotication[] = await Notification.find({ user_to: req.user._id });
...
router.get('/user/messages', auth, async (req: Request, res: Response) => {
    try {
...
router.post('/user/bill/address', auth, async (req: Request, res: Response) => {
    const address = new Address({ ...req.body, userId: req.user._id, type: 'billing' });
    try {
        const defaultBill: IAddress | null = await Address.findOne({ type: 'billing' });
        if (defaultBill) {
...
router.post('/user/address', auth, async (req: Request, res: Response) => {
    ...
    try {
        if (isDefault) {
            const defaultExist: IAddress | null = await Address.findOne({ default: true });
            if (defaultExist) {
                defaultExist.default = false;
                await defaultExist.save()
                const address: IAddress = new Address({ ...req.body, userId: req.user._id, default: true });
        ...
        const address: IAddress = new Address({ ...req.body, userId: req.user._id });
        await address.save();
        await stripe.customers.update(req.user.customer_id, {
            address: {
                country: address.country,
...
router.patch('/user/address/:id', auth, async (req: Request, res: Response) => {
    const updates: string[] = Object.keys(req.body);
    const { id } = req.params;
    const allowedUpdate = ['country', 'firstName', 'lastName', 'phoneNumber', 'type', 'default', 'address', 'id'];
    ...
    try {
        ...
        updates.forEach((update) => (existingAddress as any)[update] = req.body[update]);
...
router.delete('/user/address/:id', auth, async (req: Request, res: Response) => {
    const { id } = req.params;
    try {
...
router.patch('/user/address/default/:id', auth, async (req: Request, res: Response) => {
    const { id } = req.params;
    try {
        const defaultExist: IAddress | null = await Address.findOne({ default: true });
        if (!defaultExist) {
...
router.post('/user/payment', auth, async (req: Request, res: Response) => {
    ...
    try {
        const cart: ICart | null = await Cart.findOne({ owner: req.user._id });
        if (!cart) {
            return res.status(404).send('Nothing here');
        }
        const customer = req.user.customer_id;
        const productsArray: { price_data: { currency: string; product_data: { name: string; }; unit_amount: number; }; quantity: number; }[] = [];
        const { products } = cart;
...
router.post('/user/verify/payment', auth, async (req: Request, res: Response) => {
    try {
        ...
        console.log(id)
        const sessionDetails: Stripe.Response<Stripe.Checkout.Session> = await stripe.checkout.sessions.retrieve(id as string);
        if (!sessionDetails) {
            return res.status(404).send()
        }
        ...
        const cart: ICart | null = await Cart.findOne({ owner: req.user._id });
        await addToOrders(cart, req.user.email, req.user._id, initialAddress[0]?.address, shipping);
...
router.post('/user/close', auth, async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (store) return res.status(400).send('You cant close this account as you already have a store')
...
router.patch('/user/modify', auth, async (req: Request, res: Response) => {
    const updates: string[] = Object.keys(req.body);
    const existingData: string[] = ['name', 'phone', 'password'];
...
router.get('/user/wishlist', auth, async (req: Request, res: Response) => {
    try {
        const wishlists: IWish[] = await Wish.find({ userId: req.user._id }).populate({
            path: 'productId',
            model: Product
        });
...
router.post('/user/wishlist', auth, async (req: Request, res: Response) => {
    ...
    try {
        ...
        const wishExists: IWish | null = await Wish.findOne({ userId: req.user._id, productId })
...
router.get('/brands', async (req: Request, res: Response) => {
    try {
        const brands = await Store.find({ $or: [{ account: 'brands and  agency' }, { account: 'Small Business' }] })
...
router.get('/allstores', async (req: Request, res: Response) => {
    try {
        const stores = await Store.find({ account: 'Individual Seller' }).sort({ 'sales': -1 }).limit(4)
        // console.log(stores)
...
router.get('/findstore', async (req: Request, res: Response) => {
    ...
    try {
        let store: IStore | null = await Store.findOne({ name });
        if (!store) {
            name = new_name[0] + ' ' + capitalizeFirstLetter(new_name[1])
            store = await Store.findOne({ name });
        }
        if (!store) {
            name = unCapitalizeFirstLetter(new_name[0]) + ' ' + new_name[1]
            store = await Store.findOne({ name });
        }
        const products: IProduct[] = await Product.find({ owner: store?._id, active: true }).populate({
            path: 'ratingId',
            model: Rating
        }).populate({
            path: 'owner',
        ...
        // @ts-ignore
        res.status(200).send({ products, image: store?.logo, name: store?.name, description: store?.summary })
    }
    catch (e) {
        console.log(e)
        res.status(500).send(e)
...
router.get('/user/recommendation', auth, async (req: Request, res: Response) => {
    const user = req.user;
    try {
