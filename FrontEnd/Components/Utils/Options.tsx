...
interface Ioptions {
 ...
 const Options : React.FC<Ioptions> = ({stock}) => {
   ...
   const onSubmit : SubmitHandler<ITest> = async (data, event) => {
       //     console.log(myVariant.current?.value)
       //     const newData = {
       //         variant: myVariant.current?.value,
       //         price : data.test[(data.test.length - 1)].price,
       //         stock :  data.test[(data.test.length - 1)].stock
       //     }
       //     console.log(newData);
       // }
   }
                                                ...
                                                <Grid item xs={12} sm={6} lg={3}>
                                                    ...
                                                    />
                                                        <FormHelperText sx={{color: 'red'}}>
                                                            {(watch(`test.${index}.stock`)?.reduce((index, {name}) => index + Number.parseInt(name as unknown as string), 0) > stock ) ? 'Must not be greater than stock'  : '' }
