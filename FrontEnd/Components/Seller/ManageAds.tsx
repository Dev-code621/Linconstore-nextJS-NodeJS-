...
const ManageAds: React.FC = () => {
 ...
 useTokenRefetch(adsRefetch)
  ...
  return (
    <>
      <Head>
        <title>Manage ads | seller dashboard linconstore</title>
      ...
      <Card elevation={0} sx={{ background: "#f3f2f2", mt: 1, p: 2 }}>
        <>
          ...
          >
            ...
            <Typography variant={"h6"}> {t("seller.ads.title")} </Typography>
          </Box>
          {isPromoteError && (
          ...
          <Stack direction={"row"} spacing={2}>
            <Stack>
              ...
              <Typography variant={"body1"}>
                {ads.length} of {sellerLimit} for {packageName} package
              </Typography>
            </Stack>
          </Stack>
          ...
          <Typography variant={"h6"}>{t("seller.ads.my_product")}</Typography>
          {products.length === 0 && (
